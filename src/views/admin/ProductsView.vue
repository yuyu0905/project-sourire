        ...
        <table class="table table-hover mt-4">
          ...
          <tbody>
            <tr v-for="product in products" :key="product.id">
              ...
              <td>{{ product.set + product.title }}</td>
              ...
              <td>
                <div class="btn-group">
                  <button type="button" class="btn btn-outline-gray-dark btn-sm" @click="openModal('modify', product)">
                    編輯
                  </button>
...
export default {
  ...
  methods: {
    ...
    saveProduct (product) {
      this.isLoading = true
      let api = `${VITE_API}/api/${VITE_APIPATH}/admin/product`

      if (this.operateType === 'modify') {
        api = `${VITE_API}/api/${VITE_APIPATH}/admin/product/${product.id}`
      }
